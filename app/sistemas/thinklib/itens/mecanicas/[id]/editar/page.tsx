...
export default function EditarMecanicaPage() {
  ...
  // ── Load mechanic + categories, then resolve IDs ──────────────────────────────
  useEffect(() => {
    async function load() {
      try {
        const [mechanic, catsRes] = await Promise.all([
        ...
        setPresentationText(mechanic.presentationText ?? "");
        setDescription(mechanic.description ?? "");
        setVideoUrl(mechanic.videoUrl?.trim() === " " ? "" : (mechanic.videoUrl ?? ""));
        setUnityVersion(mechanic.unityVersion ?? "");

        if (mechanic.sourceFiles && mechanic.sourceFiles.length > 0) {
          setCodeBlocks(
            mechanic.sourceFiles.map((sf) => ({
              id: sf.id,
              fileName: sf.fileName,
              content: sf.fileContent,
            }))
          );
        }

        // Existing images shown as URL previews
        if (mechanic.imageUrl) setImagePreview(mechanic.imageUrl);
        if (mechanic.gifUrl) setGifPreview(mechanic.gifUrl);

        // Resolve categoryId by matching categoryName
        const matchedCat = cats.find(
          (c) => c.name.toLowerCase() === (mechanic.categoryName ?? "").toLowerCase()
        );
        if (matchedCat) {
          setCategoryId(matchedCat.id);

          // Load types for that category and resolve typeId
          const typesRes = await MechanicTypeService.getTypesByCategory(matchedCat.id);
          const allTypes = typesRes.items ?? [];
          setTipos(allTypes);

          const matchedType = allTypes.find(
            (t) => t.name.toLowerCase() === (mechanic.typeName ?? "").toLowerCase()
          );
          if (matchedType) setTipoId(matchedType.id);
        }
      } catch (err) {
        setError("Não foi possível carregar a mecânica.");
        console.error(err);
      } finally {
  ...
  // ── Reload types when category changes (user interaction) ─────────────────────
  ...
  useEffect(() => {
    if (!categoryChangedByUser.current) return;
    if (!categoryId) { setTipos([]); return; }
  ...
  const handleSave = async () => {
    ...
    setSubmitting(true);
    try {
      await MechanicService.updateMechanic(mechanicId, {
        name: name.trim(),
  ...
  if (loadingData) {
    return (
      <div className="pt-10 pb-16 px-[29.5px] flex items-center justify-center min-h-[300px]">
        <span className="text-sm text-[#8E95A5] font-poppins">Carregando...</span>
      </div>
    );
  }
  ...
  return (
    <div className="pt-10 pb-16 px-[29.5px] flex flex-col gap-8 w-full bg-white">
      ...
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-[#1D43BE] font-poppins">Editar mecânica</h1>
        <button
      ...
      <section className="flex flex-col gap-5">
        ...
        <div className="flex flex-col gap-1.5">
          ...
          <textarea
            ...
            placeholder="Ex: Esta mecânica permite que o jogador utilize uma habilidades especial de teletransporte para atravessar..."
            rows={4}
        ...
        <div className="flex flex-col gap-1.5">
          ...
          <textarea
            ...
            placeholder="Ex: O sistema de teletransporte é ativado ao segurar o botão de ação por 2 segundos..."
            rows={5}
            className={`${inputClass} resize-none`}
          />
        </div>
